...
from patchi.core.agents.base import AgentGroup, AgentInput, AgentResult, AgentStatus
...
from patchi.core.testing.live_v2.video_recorder import VideoRecorder, RecordingConfig, TestRecording, record_test_session
            ...
            for diff in test_result.visual_regression.get("diffs", []):
                if not diff.get("passed", True):
                    result.add_finding(Finding(
                        ...
                        detail=f"Baseline comparison failed for visual test",
                    ))
