...
class BugPredictorAgent(BaseAgent):
    ...
    def _run(self, inp: AgentInput, result: AgentResult) -> None:
        ...
        for fp in inp.root.rglob("*.py"):
            ...
            if score>15:
                findings.append(make_finding(severity=Severity.LOW, file=rel, line_start=0, title=f"Predicted bug-prone ({score:.0f}) — churn {churn} complexity {comp}", description="High churn+complexity predicts next bug — review, add tests. Features: churn, complexity, bug density; sklearn fallback linear.", finding_type="bug_prediction"))
