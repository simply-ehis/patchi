    ...
    def _run(self, inp: AgentInput, result: AgentResult) -> None:
        ...
        for pat in ("*.js","*.jsx","*.ts","*.tsx"):
            for fp in safe_rglob(inp.root, pat):
                ...
                has_interval = any(_INTERVAL_RE.search(l) for l in lines)
                has_clear = any("clearInterval" in l for l in lines)
                has_emitter = any(_EMITTER_RE.search(l) for l in lines)
                has_off = "removeListener" in txt or "off(" in txt or "removeEventListener" in txt
                has_effect = any(_USEEFFECT_RE.search(l) for l in lines)
                has_effect_cleanup = "return () =>" in txt or "return function" in txt
                has_stream = any(_STREAM_RE.search(l) for l in lines)
                has_close = ".close(" in txt or ".destroy(" in txt or ".end(" in txt

                # Interval leak
                if has_interval and not has_clear:
                    for i, l in enumerate(lines,1):
                        if _INTERVAL_RE.search(l):
