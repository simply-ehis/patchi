    ...
    def _run(self, inp: AgentInput, result: AgentResult) -> None:
        ...
        try:
            ...
            if not os.environ.get("PATCHI_OFFLINE"):
                ...
                prompt = (
                f'Parse NL security query to keywords JSON {{"keywords":["auth","rate limiting"],'
                f' "intent":"find auth without rate limiting"}}. Query: {q}'
                )
                    ...
                    if "auth" in q.lower():
                        result.add_finding(
                            make_finding(
                                ...
                                file=r.file,
                                line_start=r.line,
                                title=f"Auth route without rate limiting: {r.method} {r.path}",
                                description=f"NL query '{q}' matched — add rate limiter",
                                finding_type="nl_query_match",
                            )
                        )
        except Exception as exc:  # noqa: BLE001
