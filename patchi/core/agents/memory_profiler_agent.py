                ...
                try:
                    ...
                    if base_file.exists():
                        ...
                        if prev and total > prev*1.2:
                            findings.append(make_finding(severity=Severity.MEDIUM, file=out_dir, line_start=0, title=f"Bundle size regression {out_dir}: {prev//1024}KB → {total//1024}KB (+{(total-prev)/prev*100:.0f}%)", finding_type="bundle_size_regression"))
                    # update baseline
        ...
        for fp in root.rglob("*.js"):
            ...
            if "addEventListener" in txt and "removeEventListener" not in txt and "addEventListener" in txt:
                ...
                if txt.count("addEventListener")>3:
                    findings.append(make_finding(severity=Severity.LOW, file=str(fp.relative_to(root)), line_start=0, title="Event listener leak risk — add without remove", finding_type="event_leak"))
