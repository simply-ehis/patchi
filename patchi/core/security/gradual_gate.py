...
def ratchet_check(root: Path, current_count: int, weekly_reduction: int = 5) -> tuple[bool, str]:
    ...
    data = (
        ...
        if p.exists()
        else {"baseline": current_count, "target": current_count}
    )
    ...
    if current_count > target:
        return (
            False,
            f"Ratchet fail: {current_count} > target {target} (baseline {data.get('baseline')}) — reduce"
            f" {current_count - target} findings this week",
        )
...
def set_baseline(root: Path, count: int) -> None:
    ...
    p.write_text(
        json.dumps({"baseline": count, "target": count, "ts": time.time()}, indent=2),
        encoding="utf-8",
    )
