...
JWT_LIBRARIES: set[str] = {
    "jsonwebtoken", "pyjwt", "PyJWT", "jose", "jwt", "JJWT", "firebase/php-jwt",
    "ruby-jwt", "go-jose", "jwt-go", "jsonwebtoken",
}
JWT_API_CALLS: set[str] = {
    "jwt.encode", "jwt.decode", "jwt.sign", "jwt.verify", "jwt.create", "jwt.parse",
...
class JWTSecurityAgent(BaseAgent):
    ...
    _SECRET_PATTERNS = [
        (re.compile(
         ...
         Severity.CRITICAL, "Hardcoded JWT secret in source code"),
        (re.compile(
            r'(?:secret|key)\s*[:=]\s*["\'][A-Za-z0-9+/=_-]{16,}["\']', re.I),
         Severity.HIGH, "Possible hardcoded cryptographic key"),
    ]
    ...
    def _analyze_file(self, content: str, lang: Lang, rel_path: str) -> list[Finding]:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            for pattern, severity, message in self._WEAK_ALGO_PATTERNS:
                if pattern.search(line):
                    findings.append(self._make_finding("jwt_weak_algorithm", severity, rel_path, line_num, message,
                        "Use RS256 or ES256 for distributed systems. Never allow 'none' algorithm.", stripped))

            for pattern, severity, message in self._SECRET_PATTERNS:
                if pattern.search(line):
                    findings.append(self._make_finding("jwt_hardcoded_secret", severity, rel_path, line_num, message,
                        "Move JWT secret to environment variable.", stripped))

            for pattern, severity, message in self._STORAGE_PATTERNS:
                if pattern.search(line):
                    findings.append(self._make_finding("jwt_insecure_storage", severity, rel_path, line_num, message,
                        "Use httpOnly, secure cookies for JWT storage.", stripped))

            for pattern, severity, message in self._VALIDATION_ISSUES:
                if pattern.search(line):
                    findings.append(self._make_finding("jwt_missing_validation", severity, rel_path, line_num, message,
                        "Always verify JWT signatures and specify allowed algorithms.", stripped))
        ...
        if has_jwt:
            ...
            if not has_expiry:
                findings.append(
                    make_finding(
                        agent=self.name, finding_type="jwt_no_expiry",
                        severity=Severity.HIGH, file=rel_path,
    ...
    def _make_finding(self, finding_type: str, severity: Severity, file: str, line: int, message: str, suggestion: str, snippet: str) -> Finding:
        return make_finding(
            agent=self.name, finding_type=finding_type,
            severity=severity, file=file, line_start=line,
    ...
    def _check_no_jwt_in_codebase(self, content: str, rel_path: str) -> list[Finding]:
        findings = []
        env_patterns = [
            (re.compile(r'JWT_SECRET\s*=\s*["\'][^"\']+["\']', re.I), Severity.CRITICAL,
             "JWT secret in environment/config file — use .env, not hardcoded"),
            (re.compile(r'JWT_KEY\s*=\s*["\'][^"\']+["\']', re.I), Severity.HIGH,
             "JWT key in environment/config file — ensure .env is in .gitignore"),
        ]
        for line_num, line in enumerate(content.splitlines(), 1):
            for pattern, severity, message in env_patterns:
                if pattern.search(line):
                    findings.append(self._make_finding("jwt_env_exposure", severity, rel_path, line_num, message,
                        "Ensure .env files are in .gitignore. Use secret managers in production.", line.strip()))
        return findings
