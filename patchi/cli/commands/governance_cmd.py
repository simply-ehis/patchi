...
def _stamp() -> str:
    ...
    Same canonical `p <args>` form the reason_cmd reports use (no
    machine-specific entrypoint path, so the stamp reads clean when pasted
    into a PR or doc).
    """
    from datetime import UTC, datetime

    return f"_Generated by `p governance impact --out` on {datetime.now(UTC).strftime('%Y-%m-%d %H:%M UTC')}_"
...
def _write_out(out: Path, content: str) -> None:
    ...
    pastable text (same contract as reason_cmd._write_out).
    """
    out = Path(out)
...
def _impact_markdown(result: dict) -> str:
    """``--out`` document for ``p governance impact``: the blast-radius answer
    in PR-ready markdown — headline, reference table, and an explicit note
    when the file list was truncated."""
    count = result["reference_count"]
    files = result["referencing_files"]
    shown = files[:15]
    lines = [
        f"# Blast radius — `{result['symbol']}`",
        "",
        _stamp(),
        "",
        f"**{count} reference(s)** across **{len(files)} file(s)** — blast level: **{result['blast_level']}**.",
        "",
    ]
    if not files:
        lines.append("No referencing files found — nothing in the repo imports or uses this symbol.")
    else:
        lines.extend(
            [
                "| # | Referencing file |",
        ...
        for i, f in enumerate(shown, 1):
            lines.append(f"| {i} | `{f}` |")
        if count > len(shown):
            lines.extend(["", f"_…and {count - len(shown)} more reference(s) not listed._"])
    lines.append("")
    return "\n".join(lines)


def _cmd_impact(root: Path, symbol: str, json_output: bool, out: Path | None = None) -> int:
    from patchi.core.security.blast_radius import patchi_blast_radius

    if out is not None and json_output:
        print("--out and --json are mutually exclusive — pick one", flush=True)
        return 2

    result = patchi_blast_radius(symbol, root)

    if out is not None:
        _write_out(out, _impact_markdown(result))
        # Plain print, NOT con.print: Rich soft-wraps at console width,
        # which can split a long file path mid-string — a wrapped path
        # can't be copied back or parsed (same corruption class _emit
        # guards against for JSON).
...
def run(args: argparse.Namespace) -> int:
    ...
    if sub == "impact":
        ...
        if not symbol:
            ...
            return 1
        return _cmd_impact(root, symbol, json_output, getattr(args, "out", None))
    if sub == "triage":
