...
Subcommands:
  p mode            — show current mode
  p mode confirm    — require approval for every action
  p mode auto       — auto-apply safe fixes, ask for risky ones
  p mode autopilot  — full trust, Patchi decides everything
"""
...
def run(mode_str: str | None = None, root: Path | None = None) -> None:
    ...
    `p mode` alone shows the current mode, `p mode <name>` sets it. Existing
    behavior, just no longer split across two dispatch branches in main.py.
    """
    if mode_str:
