...
COMMANDS: list[Command] = [
    Command(
        ...
        args=(
            Arg("--no-logo", dest="no_logo", action="store_true",
                help="Skip logo draw (for CI)"),
        ),
    ),
    Command(
        "health",
    ...
    Command(
        ...
        args=(
            ...
            Arg("--all-flows", dest="all_flows", action="store_true",
            ...
            Arg("--no-side", dest="side", action="store_false", default=True,
                help="Skip side file scanners (faster, source-only scan). Default: on"),
            Arg("--pipeline", action="store_true",
    ...
    Command(
        ...
        args=(
            ...
            Arg("--interval", dest="interval", type=float, default=15.0,
                help="Seconds between full health/drift/fix refreshes"),
            Arg("--poll", dest="poll", type=float, default=1.0,
                help="Seconds between file-change polls"),
            Arg("--once", dest="once", action="store_true",
                help="Render one frame and exit (CI / smoke test)"),
            Arg("--scan-secrets", dest="scan_secrets", action="store_true",
    ...
    Command(
        ...
        subcommands=(
            ...
            Command("skip", "Skip the current active task", "patchi.cli.commands.queue_cmd:run_skip"),
            Command("clear", "Remove all waiting tasks", "patchi.cli.commands.queue_cmd:run_clear"),
            Command(
                "mode", "Set queue mode", "patchi.cli.commands.queue_cmd:run_set_mode",
    ...
    Command(
        ...
        subcommands=(
            Command("list", "List all patches", "patchi.cli.commands.patch_cmd:run_list"),
    ...
    Command(
        ...
        subcommands=(
            ...
            Command("remove", "Remove a key by nickname", "patchi.cli.commands.key_cmd:run_remove",
                     args=(Arg("nickname"),)),
    ...
    Command("undo", "Undo last applied fix or specific patch", "patchi.cli.commands.undo_cmd:run_undo",
            args=(Arg("patch_id", nargs="?", help="Specific patch ID"),)),
    Command("redo", "Redo last undone fix or specific patch", "patchi.cli.commands.undo_cmd:run_redo",
            args=(Arg("patch_id", nargs="?", help="Specific patch ID"),)),
    Command("rollback", "Roll back to a specific patch", "patchi.cli.commands.undo_cmd:run_rollback",
            args=(Arg("patch_id"),)),
    ...
    Command("why", "Explain why a file matters", "patchi.cli.commands.reason_cmd:run_why",
            args=(Arg("path", help="File path to explain"),)),
    Command("impact", "Show change-impact / blast radius for changed files",
    ...
    Command("auto", "Propose/apply safe fixes for changed files", "patchi.cli.commands.auto_cmd:run",
            args=(Arg("files", nargs="+", help="One or more changed file paths"),
                  ...
                  Arg("--reject", dest="reject", default=None,
    ...
    Command("verify", "Independently re-run tests+scan and report truth (not self-report)",
            "patchi.cli.commands.verify_cmd:run",
            args=(Arg("--no-scan", dest="no_scan", action="store_true",
                      help="Only re-run tests, skip scan"),
                  Arg("--claim", dest="claim", default=None,
                      help="Assert a claim (e.g. 'tests pass') against actual truth"))),
    Command("mode", "View or set operating mode", "patchi.cli.commands.mode_cmd:run",
            args=(Arg("mode_str", nargs="?", choices=("confirm", "auto", "autopilot"),
                      help="New mode"),)),
    Command("chat", "Interactive AI chat with Patchi", "patchi.cli.commands.chat_cmd:run",
    ...
    Command("blast", "Blast radius — what breaks when you change a file",
            "patchi.cli.commands.blast_cmd:run",
            args=(Arg("file_path", nargs="?", help="File to analyze"),
                  # drift fix: parser used --all (dest "all"), handler wants show_all
                  Arg("--all", dest="show_all", action="store_true",
                      help="Show blast radius for all files"))),
    ...
    Command("trend", "Health and quality trend over time", "patchi.cli.commands.trend_cmd:run",
            args=(Arg("metric", nargs="?", choices=("security", "tests"),
    ...
    Command("log", "Show git changelog", "patchi.cli.commands.log_cmd:run",
            args=(Arg("--since", default="HEAD~10", help="Git ref or date (default: HEAD~10)"),)),
    Command("update", "Check for and apply updates", "patchi.cli.commands.update_cmd:run",
            args=(Arg("--check", action="store_true", help="Check only, no install"),
    ...
    Command("learn", "Learn project conventions and patterns", "patchi.cli.commands.learn_cmd:run",
            args=(Arg("--force", action="store_true",
    ...
    Command(
        ...
        subcommands=(
            Command("list", "List agent groups or agents in a group",
                    "patchi.cli.commands.agents_cmd:run_list",
                    args=(Arg("group_name", nargs="?"),)),
            Command("status", "Show currently active agents",
                    "patchi.cli.commands.agents_cmd:run_status"),
            Command("reset", "Reset circuit breaker for an agent (LIMIT-04)",
                    "patchi.cli.commands.agents_cmd:run_reset",
                    args=(Arg("agent_name", nargs="?", help="Agent name to reset (omit for all)"),)),
        ),
    ),
    Command(
        "model", "Manage local Ollama model",
        "patchi.cli.commands.model_cmd:run_status",  # no subcommand -> status, matches original
        subcommands=(
            Command("set", "Set local model", "patchi.cli.commands.model_cmd:run_set",
                    args=(Arg("model_name"),)),
            Command("list", "List available local models", "patchi.cli.commands.model_cmd:run_list"),
            Command("status", "Show model connection health", "patchi.cli.commands.model_cmd:run_status"),
        ),
    ),
    Command(
        "memory", "View and manage Patchi's memory",
        "patchi.cli.commands.memory_cmd:run_show_all",  # no subcommand -> show_all, matches original
        subcommands=(
            Command("show", "Show memory for a category", "patchi.cli.commands.memory_cmd:run_show",
                    args=(Arg("category_str"),)),
            Command("delete", "Clear memory (category or all)",
    ...
    Command("plan", "Plan changes before making them", "patchi.cli.commands.plan_cmd:run",
            args=(Arg("area", nargs="?", help="Targeted area — plain language or path"),
                  ...
                  Arg("--json", dest="json_output", action="store_true"),
                  Arg("--html", dest="html", default=None,
                      help="Write a self-contained HTML report"))),
    Command(
        "restrict", "Manage file restrictions",
        "patchi.cli.commands.restrict_cmd:run_list",  # no subcommand -> list, matches original
        subcommands=(
            Command("add", "Add path as no-touch zone", "patchi.cli.commands.restrict_cmd:run_add",
                    args=(Arg("path"), Arg("--reason", dest="reason", default="")),
                    fixed_kwargs={"rtype": _RestrictionType.NO_TOUCH}),
            Command("scan-only", "Mark path as scan-only", "patchi.cli.commands.restrict_cmd:run_add",
                    args=(Arg("path"), Arg("--reason", dest="reason", default="")),
                    fixed_kwargs={"rtype": _RestrictionType.SCAN_ONLY}),
            Command("sensitive", "Mark path as sensitive", "patchi.cli.commands.restrict_cmd:run_add",
                    args=(Arg("path"), Arg("--reason", dest="reason", default="")),
            ...
            Command("remove", "Remove a restriction", "patchi.cli.commands.restrict_cmd:run_remove",
                    args=(Arg("path"),)),
            Command("disable", "Temporarily disable a restriction",
                    "patchi.cli.commands.restrict_cmd:run_disable", args=(Arg("path"),)),
            Command("enable", "Re-enable a restriction",
                    "patchi.cli.commands.restrict_cmd:run_enable", args=(Arg("path"),)),
        ),
    ),
    ...
    Command("report", "Generate a structured analysis report",
            ...
            args=(Arg("report_cmd", nargs="?", choices=("export", "weekly"),
                      help="export | weekly"),
                  Arg("--format", dest="fmt", default="markdown",
                      help="Output format (markdown | json)"))),

    Command("chain", "Exploit chain analysis over current findings",
            "patchi.cli.commands.chain_cmd:run",
            args=(Arg("--min-severity", dest="min_severity", default="medium",
                      choices=("info", "low", "medium", "high", "critical"),
                      help="Minimum chain severity to display"),
                  Arg("--json", dest="json_output", action="store_true",
                      help="JSON output"),
                  Arg("--max", dest="max_chains", type=int, default=20,
                      help="Max chains to show")),
            ),
    Command("intent", "Business-logic / access-control route analysis",
    ...
    Command("assure", "Assurance campaigns — prove properties, record evidence",
            ...
            args=(Arg("--json", dest="json_output", action="store_true",
                  ...
                  Arg("--run-attackers", dest="run_attackers", action="store_true",
                      help="Run adversarial attacker hypotheses against the assurance graph"),
    ...
    Command("audit", "Full project audit + Plan-vs-Built drift",
            ...
            args=(Arg("--quick", action="store_true", help="Quick audit (skip deep scan)"),
                  Arg("--json", dest="json_output", action="store_true",
                      help="JSON output"),
                  Arg("--plan", action="store_true",
                      help="Snapshot current state as the agreed Plan"),
                  Arg("--plan-file", dest="plan_file", type=str,
                      help="Audit built state against a plan file"),
                  Arg("--intent", type=str,
                      help="Intent description (with --plan)"),
                  Arg("--no-scan", dest="no_scan", action="store_true",
                      help="Skip scan during audit"),
                  Arg("--html", type=str, help="Write self-contained HTML report"))),

    Command("settings", "View or modify configuration",
            "patchi.cli.commands.settings_cmd:run_show",  # no subcommand -> show
            subcommands=(
                Command("show", "Show current configuration",
                        "patchi.cli.commands.settings_cmd:run_show"),
                Command("set", "Set a configuration value",
                        "patchi.cli.commands.settings_cmd:run_set",
                        args=(Arg("key"), Arg("value"))),
            )),

    Command("access", "Manage dev access tokens",
            "patchi.cli.commands.access_cmd:run_list",  # no subcommand -> list
            subcommands=(
                Command("add", "Add a token", "patchi.cli.commands.access_cmd:run_add",
                        args=(Arg("name"), Arg("--env-var", dest="env_var"))),
                Command("list", "List all tokens",
                        "patchi.cli.commands.access_cmd:run_list"),
                Command("remove", "Remove a token",
    ...
    Command("ai", "AI configuration and status",
            ...
            subcommands=(
                Command("status", "Show all configured AI keys and status",
    ...
    Command("charter", "Manage project charter and rules",
            ...
            subcommands=(
                Command("show", "Show current charter",
                        "patchi.cli.commands.charter_cmd:run_show"),
                Command("set", "Set charter from a file",
                        "patchi.cli.commands.charter_cmd:run_set",
                        args=(Arg("text"), Arg("--ai", dest="use_ai", action="store_true",
                              help="Use AI to generate charter"))),
                Command("check", "Check code against charter",
                        "patchi.cli.commands.charter_cmd:run_check",
                        args=(Arg("--rebuild", action="store_true",
                              help="Rebuild charter index"),)),
    ...
    Command(
        ...
        subcommands=(
            ...
            Command("add", "Add a notification channel",
                    args=(Arg("channel_type",
                              choices=("email", "slack", "discord", "webhook", "telegram")),)),
            Command("remove", "Remove a channel", args=(Arg("channel_name"),)),
            Command("test", "Send test alert to all or one channel",
                    args=(Arg("channel_name", nargs="?"),)),
            Command("ack", "Acknowledge a CRITICAL/HIGH alert",
                    args=(Arg("alert_id"),)),
    ...
    Command(
        ...
        subcommands=(
            ...
            Command("daemon", "Start worker with auto-restart + health checks",
                    args=(Arg("--guard", action="store_true",
    ...
    Command(
        "test", "Run tests, generate suites, view history",
        "patchi.cli.commands.test_cmd:run_test",
        namespace_handler=True,
        args=(
            Arg("type", nargs="?",
                choices=("unit", "browser", "stress", "regression", "accessibility",
                         "api", "buttons", "layout", "e2e", "visual", "smoke", "full",
                         "security", "generate", "report", "config"),
    ...
    Command(
        ...
        args=(
            Arg("type", nargs="?",
                choices=("taint", "secrets", "config", "headers", "ratelimit", "cors",
                         "deps", "probe", "jwt", "sensitive", "auth", "ssrf", "injection",
                         "authz", "crypto", "network", "privacy", "depvuln", "compliance",
                         "secrets_guard", "supply", "iac", "container", "policy", "cve",
                         "redteam", "precheck", "runtime", "appmap", "browsertest", "evidence",
                         "blast", "governance", "history", "report", "cdn", "dns", "emailauth",
                         "push", "saml", "secrets_runtime", "mesh", "k8s"),
                help="Scan type"),
            Arg("area", nargs="?"),
            Arg("--policy", type=str,
    ...
    Command("cross-repo", "Cross-repository dependency intelligence",
            "patchi.cli.commands.cross_repo_cmd:run",
            namespace_handler=True,
            args=(Arg("action", nargs="?", default="scan",
    ...
    Command(
        ...
        args=(
            Arg("--max-loops", type=int, default=5,
                help="Maximum pipeline iterations (default 5)"),
            Arg("--target", type=int, default=100,
                help="Health score target to reach (default 100)"),
            Arg("--dry-run", action="store_true",
