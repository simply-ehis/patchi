...
<style>
body {{ font-family: ui-sans-serif, system-ui, sans-serif; margin: 0; background: #0d1117; color: #e6edf3; padding:
  2rem; }}
...
async def brain_map_page(request: Request):
    ...
    return templates.TemplateResponse(request, "brain.html", {"request": request, "layers_data": layers_data})
...
async def attack_timeline_page(request: Request):
    ...
    return templates.TemplateResponse(request, "attacks.html", {"request": request, "attack_data": attack_data})
...
async def hosted_page(request: Request):
    ...
    return templates.TemplateResponse(request, "hosted.html", {"request": request, "hosted_data": hosted_data})
        ...
        commands.append(
            {
                ...
                "subcommands": [{"name": sc.name, "help": sc.help} for sc in cmd.subcommands]
                if cmd.subcommands
                else [],
            }
        )
